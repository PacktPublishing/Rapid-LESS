...
function init_Elements () {
   ...
   //Init 2 type tabs
    ...
    //Mega select interaction
            $('.mega-select-filter').click(function(e){
                //prevent the default behaviour of the link
                e.preventDefault();
                $('.select-field').val('');
   
                $('.mega-select-filter').removeClass('filter-active');
                ...
                $('.select-btn a:not(.' + filter + ')').hide();

                //Filter dropdown
                //show all the list items(this is needed to get the hidden ones shown)
                $(".select-group").removeClass("active-dropdown");
                $(".select-group").show();
                
                /*using the :not attribute and the filter class in it we are selecting
                only the list items that don't have that class and hide them '*/
                $('.select-group.' + filter).addClass("active-dropdown");
                $('.select-group:not(.' + filter + ')').hide();
            });
...
function init_Home() {
    ...
    //Mega select interaction
                    $('.mega-select-filter').click(function(e){
                        ...
                        //show all the list items(this is needed to get the hidden ones shown)
                        $(".select-btn a").show();
                        $(".select-btn a").css('display', 'inline-block');
                    ...
                    $('.select-variant').click( function (e) {
                        e.preventDefault();
                        $(this).parent().find('.active-dropdown').css("z-index", '50');
                        var value = $(this).attr('data-value');
                        $('.select-field').val(value);
    ...
    //scroll down
    $('.movie-top-check').click(function (ev) {
        ev.preventDefault();
...
(function(d, s, id) {
  ...
  js.src = "//connect.facebook.net/en_US/sdk.js#xfbml=1&appId=733016180051725&version=v2.0";
